...
function groupByCategory(programs: ProgramListItem[]): [string, ProgramListItem[]][] {
  const groups = new Map<string, ProgramListItem[]>()
...
/**
 * Desktop "Programs" nav item — opens a mega menu listing every published
 * training program from the LMS Course Management API, grouped by
 * category, instead of navigating straight to `/programs`. Supports hover,
 * click, and keyboard (focus + Escape) — never relies on CSS `:hover`
 * alone. Fetch failures here degrade silently to an empty menu (matching
 * the previous static-data-era behavior) rather than surfacing an error
 * inside a nav dropdown — `ProgramsPage.tsx` itself has the real
 * loading/error UI for this data.
 */
export function ProgramsMegaMenu() {
                      ...
                      <li key={program.slug}>
                        ...
                        >
                          <span className="bg-primary-soft text-primary-dark flex size-8 shrink-0 items-center justify-center rounded-full">
                            <ProgramIcon slug={program.slug} className="size-4" />
                          </span>
                          {program.title}
                        </Link>
                      </li>
