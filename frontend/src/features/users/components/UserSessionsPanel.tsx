...
import {
  ...
  useUserSessions,
} from '@/features/users/hooks/use-user-sessions'
...
export function UserSessionsPanel({ userId, email }: { userId: string; email: string }) {
  const sessionsQuery = useUserSessions(userId)
  const forceLogoutSession = useForceLogoutSession(userId)
  const forceLogoutAll = useForceLogoutAll(userId)
    ...
    <div className="flex flex-col gap-3">
      ...
      {sessions.length === 0 ? (
        <EmptyState
          ...
          description={`${email} is not signed in anywhere.`}
        />
      ) : (
      ...
      <ConfirmDialog
        ...
        description={`${email} will be signed out of every device.`}
        tone="destructive"
