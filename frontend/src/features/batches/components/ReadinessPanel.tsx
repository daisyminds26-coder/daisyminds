...
import type { ReadinessResult } from '@/features/batches/types'
...
/**
 * Backend readiness response is the source of truth — this only renders it,
 * never re-derives blockers client-side. `blocker.code` is intentionally
 * unused here (the human-readable `message` is sufficient for this panel).
 */
  ...
  if (readiness.ready) {
    return (
      <div className="border-success/30 bg-success/5 flex items-center gap-2 rounded-lg border p-3">
        ...
        <p className="text-body-sm font-medium">Ready to schedule</p>
      </div>
    )
  }
