...
import {
  ...
  useStudentSessions,
} from '@/features/students/hooks/use-student-sessions'
...
export function StudentSessionsPanel({ studentId, name }: { studentId: string; name: string }) {
  const sessionsQuery = useStudentSessions(studentId)
  const forceLogoutSession = useForceLogoutSession(studentId)
  const forceLogoutAll = useForceLogoutAll(studentId)
