...
interface EligibleTrainersFieldProps<TFieldValues extends FieldValues> {
...
/**
 * Course metadata only — selecting a trainer here does not assign them to
 * the course, create a batch relationship, or affect their schedule (task's
 * own explicit boundary). Reuses `features/trainers`' own read API (`GET
 * /trainers`) rather than duplicating a trainer list/search implementation
 * — the trainers module already excludes deleted/inactive trainers from
 * this endpoint by default.
 */
export function EligibleTrainersField<TFieldValues extends FieldValues>({
  control,
  name,
  label,
  description,
}: EligibleTrainersFieldProps<TFieldValues>) {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState('')

  const trainersQuery = useQuery({
    queryKey: ['trainers', 'eligible-search', search],
