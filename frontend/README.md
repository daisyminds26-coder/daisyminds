...
`features/courses/` against `../backend/src/routes/course.routes.ts`. Entry point: `pages/admin/CoursesPage.tsx`, reachable via the "Courses" nav item for `ADMIN`/`SUPER_ADMIN`. Scope is course-level metadata and lifecycle only — no curriculum, lessons, video, batches, Enrollment, or payments; a course is a single self-contained document, not a `users`-linked profile like students/trainers. See ARCHITECTURE.md §19, DATABASE.md §3.3, SECURITY.md §3/§4/§6/§8 for the full design.
...
- The detail page's future-module tabs (curriculum, batches, Enrollment, etc.) each show an honest "available in a later phase" empty state — never fabricated data, per this module's own hard requirement.
...
7. Open the new course's detail page — confirm the future-module tabs (curriculum/batches/Enrollment/etc.) each show a clear "available in a later phase" empty state, not a broken control or fake data.
...
`features/batches/` against `../backend/src/routes/batch.routes.ts`. Entry points: `pages/admin/BatchesPage.tsx` (`/admin/batches`), `pages/admin/BatchCreatePage.tsx` (`/admin/batches/new`), `pages/admin/BatchDetailPage.tsx` (`/admin/batches/:batchId`), reachable via the "Batches" nav item for `ADMIN`/`SUPER_ADMIN`. A batch is a scheduled delivery *instance* of a course — "when and how," as distinct from a course's own "what." This module deliberately does not implement attendance, live classes, or anything downstream of scheduling — student Enrollment, waitlisting, and batch transfer now live in the **Admin Enrollment Operations** module below (Phase 10B Part 2), with a lightweight Students tab surfaced here on the batch detail page. See ARCHITECTURE.md §22, DATABASE.md §3.3, SECURITY.md §3/§4/§8, API-STANDARDS.md for the full design.
...
- The batch list and detail page's capacity display reads "{occupied}/{max} seats" — real, backend-computed seat occupancy (Phase 10B Part 2), not the earlier "Max {N}" placeholder. `occupiedSeats` is never client-editable; the only way to change it is through the Enrollment Operations module's own lifecycle actions.
...
1. `/admin/batches` loads the stat cards (total active/scheduled, upcoming, active, draft — never a student/Enrolllled/seat count), table, filter bar, and pagination; search (name/batch code), filters (status/delivery mode/temporal), and sort all work.
...
- **Roster** (Students tab) — search and the status filter both narrow the table; a row's action menu only offers actions valid for that Enrollment's current status (e.g. only `ACTIVE` rows offer Suspend/Complete); Suspend/Resume/Complete/Transfer/Drop all work and match `EnrollllmentDetailPage`'s own behavior exactly (same mutations).
...
`features/Enrollllments/` against `../backend/src/routes/Enrollllment.routes.ts`. Entry points: `pages/admin/EnrollllmentsPage.tsx` (`/admin/Enrollllments`), `pages/admin/EnrollllmentCreatePage.tsx` (`/admin/Enrollllments/new`), `pages/admin/EnrollllmentDetailPage.tsx` (`/admin/Enrollllments/:EnrollllmentId`), reachable via the "Enrollllments" nav item for `ADMIN`/`SUPER_ADMIN`. An Enrollment is a student's membership in one batch — the admin surface over Phase 10B Part 1's backend engine (capacity/waitlist, lifecycle, transfer, access entitlement), which this build treats as authoritative and does not redesign. This module deliberately does not implement the Student Learning Portal, self-service Enrollment, the Learning Player, progress/attendance/assignments/quizzes, fees/payments, or certificates. See ARCHITECTURE.md §24, DATABASE.md §3.4, SECURITY.md §3/§4/§8 for the full design.

- The create flow (`CreateEnrollllmentWizard.tsx`) is a 4-step `Stepper`-driven form (Student → Batch → Capacity → Review), same one-`useForm`-instance-across-steps pattern as every other admin wizard. The Capacity step shows live seat availability and previews the outcome ("a seat is available" vs. "batch full — student will be waitlisted"), but the server decides the actual result at submit time — the UI never assumes its own preview is authoritative, and the result screen distinguishes a Confirmed toast from a Waitlisted one.
- `StudentSelectorField`/`BatchSelectorField`/`StudentMultiSelectField` are read-only, cross-feature reuse of `features/students/api/students.api.ts#listStudents` and `features/batches/api/batches.api.ts#listBatches` — the same combobox template `PrimaryTrainerField`/`EligibleTrainersField` established. The backend remains authoritative on Enrollment eligibility (duplicate/suspended-account/capacity); a non-`ACTIVE` student is shown with a visible status flag rather than hidden, so an admin understands why a submit might later be rejected.
...
- Lifecycle actions on the detail page and the list's row-action menu are both computed from the Enrollment's current status (never a generic status dropdown) — only the transitions valid from that status are ever offered, mirroring the backend's own `ALLOWED_TRANSITIONS` table without duplicating its logic. All consequential actions (Suspend/Complete/Drop/Cancel/Promote/Transfer) require a confirmation dialog naming the actual consequence.
- `TransferEnrollllmentDialog` restricts its target-batch selector to the Enrollment's own course (`BatchSelectorField courseId={Enrollllment.courseId}`) and excludes the current batch — the backend re-verifies the same-course rule regardless, so this is a UX guardrail, not the security boundary. A successful transfer shows lineage links ("Transferred to:"/"Transferred from:") on both the old (now `DROPPED`) and new Enrollment.
- `BulkEnrollllDialog` shows an *estimate* of confirmed vs. waitlisted counts before submit, clearly labeled as such — the backend decides the real, transaction-safe per-student result, and the result screen (succeeded/waitlisted/failed with per-student reasons) treats a partial failure as normal, never a top-level error.
...
- The Batch Detail "Students" tab and Student Detail "Enrollllments" tab are both lazy-loaded (only query once their tab is actually opened) and intentionally lightweight — a capacity/waitlist summary and a capped recent-Enrollllments list linking to the full `/admin/Enrollllments` list, not a full roster/history browser.
...
1. `/admin/Enrollllments` loads the stat cards (Active/Confirmed/Waitlisted/Suspended), table, filter bar, and pagination; search (Enrollment code/student name/email/batch/course) and the status/source/sort filters all work.
2. Click "Enrolll Student" — the 4-step wizard opens. Select a student with an available-seat batch — the Capacity step shows "a seat is available," and submitting shows a Confirmed outcome; land on the new Enrollment's detail page with a generated code (`DM-ENR-{year}-{6 digits}`) and `CONFIRMED` status.
...
4. Attempt to Enrolll a student already actively Enrolllled in the same batch — confirm a clear "already Enrolllled" error is surfaced (not a raw error code), and no duplicate Enrollment record is created.
...
6. From a `WAITLISTED` Enrollment (list row menu or detail page), click "Promote from waitlist" — confirm it succeeds when a seat is free and shows a clear "No seat is currently available" message when it isn't; confirm the batch's capacity/waitlist numbers refresh afterward.
7. Suspend an `ACTIVE` Enrollment — confirm the seat remains counted as occupied (capacity numbers don't change) and the Access badge switches to "Access Suspended"; Resume — confirm it returns to "Access Active."
8. Complete an `ACTIVE` Enrollment — confirm its seat is released (the batch's available-seats count increases by one) and the Access badge reflects lifetime/ended access correctly depending on whether an access end date is set.
9. From a `CONFIRMED`/`ACTIVE`/`SUSPENDED` Enrollment, click "Transfer" — confirm the target-batch selector only ever shows batches from the same course (never a cross-course batch, even if one exists); complete a transfer and confirm the original Enrollment shows `DROPPED` with a "Transferred to:" link, and the new one shows "Transferred from:" — both links navigate correctly.
...
13. Export CSV with a filter applied — file downloads, respects the filter, and the filename matches `daisy-minds-Enrollllments-YYYY-MM-DD.csv`.
14. Open an Enrollment's Audit tab — confirm entries appear only once the tab is opened (not pre-fetched with the rest of the page) and list human-readable actions (Enrolllled/Activated/Suspended/etc.), not raw event metadata.
15. On a batch's detail page, open the "Students" tab — confirm it shows real seat/available/waitlisted counts, a capped list of currently Enrolllled students, a working "Add Student" action pre-scoped to that batch, and a "View all Enrollllments" link that lands on the Enrollllments list pre-filtered to that batch.
16. On a student's detail drawer, open the "Enrollllments" tab — confirm it lazy-loads (no request fires until the tab is opened), shows the student's Enrollment history with course/batch/status, and each row links to that Enrollment's own detail page.
...
18. Try to reach `/admin/Enrollllments` while logged in as a `STUDENT` or `TRAINER` — redirected away (same `RequireRole` gate every other admin page uses).
...
- Every query hook (`useStudentDashboard`, `useStudentEnrollllments`, `useStudentCourse`, `useStudentSchedule`, `useStudentResources`, `useStudentProfile`) reads only the authenticated student's own data — there is no `studentId` parameter to pass in anywhere in this feature, by construction.
- `CourseCard`/`StudentCourseOverviewPage` render the backend-derived `accessState` (`accessStateLabel`/`accessStateTone` in `utils/access-state.ts`) via the shared `StatusBadge` — same "never re-derive entitlement client-side" rule the Admin Enrollment module's `AccessBadge` already established.
...
- `StudentProfilePage`'s edit form is deliberately narrow — phone/alternate phone/address/one emergency contact only, matching exactly what `backend/src/validators/student-portal.validator.ts#updateOwnProfileSchema` accepts; `studentId`, Enrollllment, role, account status, and profile photo upload are read-only or out of scope this phase.

### Manual testing checklist

Logged in as `STUDENT` (the seeded `active@example.com` / `correct-horse-1` account, or any student created via Admin Student Management with an admin-created Enrollment):

1. `/student` loads the dashboard — with an `ACTIVE` Enrollment, "Continue Learning" shows the real course/batch and its link lands on the correct course overview; with no Enrollment at all, an honest "You don't have an active course yet" empty state appears instead (no fabricated stat cards).
2. `/student/courses` lists every Enrollment as a card (thumbnail/title/code/batch/access-state badge/level/mode/certificate indicator) — never a course the student isn't Enrolllled in.
3. Open a course from either page — `/student/courses/:courseId` shows the header (title, level, mode, batch, trainer) plus the published curriculum accordion when the Enrollment is `ACTIVE`/`COMPLETED`-with-access.
4. From an Admin session, suspend that same student's Enrollment; reload the course page as the student — confirm it still returns `200` with an "Your course access is currently paused" message, the curriculum section is gone, and no lesson/module data is present in the response (check Network tab, not just the UI).
5. Visit a course id the student was never Enrolllled in — confirm a clean 404-driven "Course not found" state, not a raw error or a leaked course title.
...
11. From an Admin session, suspend the student's Enrollment; reload a lesson the student was mid-way through — confirm the lesson shows a paused-access state, not the video/text content, and the media-delivery endpoint returns `403` if called directly.
12. As the student, edit the page URL to a lesson id from a course you're not Enrolllled in — confirm `404`, not a leaked lesson title or a raw error.
...
8. Log in as an Enrolllled `STUDENT` — confirm `/student/live-classes` shows the session with no Join button until the 15-minute window opens (or seed one already `LIVE`), that clicking Join opens the meeting URL in a new tab, and that a cancelled session shows a clear cancelled state instead of disappearing.
