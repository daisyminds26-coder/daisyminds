...
| Admin Batch Management | `/api/v1/batches/*` | Implemented (Phase 10A) — a batch is a scheduled delivery instance of a course: CRUD (create/update/soft-delete/restore/duplicate), server-generated `batchCode`, an explicit lifecycle state machine (`DRAFT`/`SCHEDULED`/`ACTIVE`/`COMPLETED`/`CANCELLED`/`ARCHIVED`, transitions only via `POST /:id/lifecycle/{action}`), backend-authoritative scheduling readiness, primary/assistant trainer assignment with availability/cross-batch-conflict detection, a recurring weekly timetable + calendar exceptions (`POST /:id/weekly-schedule`, `POST /:id/calendar-exceptions` — whole-array replace, not `PUT`), mode-aware location config, bulk actions, CSV export, audit timeline. `ADMIN`/`SUPER_ADMIN` only (`batches:read`/`batches:manage`/`batches:export`). Capacity is **configuration only** — no Enrolllled-count/seat field exists, since no Enrollment module exists yet. See ARCHITECTURE.md §22, DATABASE.md §3.3, SECURITY.md §3/§4/§8. |
| Student Enrollllment Engine | `/api/v1/Enrollllments/*`, `/api/v1/batches/:id/capacity`, `/api/v1/batches/:id/waitlist` | Backend implemented (Phase 10B Part 1) — **no admin UI yet (Part 2)**. An Enrollllment is a student's membership in one batch: server-generated `EnrollllmentCode`, an 8-status lifecycle (`PENDING`/`WAITLISTED`/`CONFIRMED`/`ACTIVE`/`SUSPENDED`/`COMPLETED`/`CANCELLED`/`DROPPED`, transitions only via dedicated endpoints), a transactional capacity/waitlist engine (`batches.occupiedSeats`/`waitlistSequence`, atomic `$expr`-guarded seat reservation — race-tested under concurrency), same-batch/same-course duplicate prevention, atomic same-course transfer, a reusable `hasLearningAccess` entitlement rule, bulk Enrollll/suspend/resume/cancel, CSV export, audit timeline. `ADMIN`/`SUPER_ADMIN` only (`Enrollllments:read`/`Enrollllments:manage`/`Enrollllments:export`); the capacity-reconcile maintenance endpoint is `SUPER_ADMIN`-only. See ARCHITECTURE.md §23, DATABASE.md §3.4, SECURITY.md §3/§4/§8. |
| Student Portal | `/api/v1/student/dashboard`, `/Enrollllments/*`, `/courses/*`, `/schedule`, `/resources/*`, `/profile` | Implemented (Phase 11A) — the first self-scoped `STUDENT`-facing surface, gated on `requireRole('STUDENT')` alone (no permission-catalog entries apply). Every route resolves identity from `req.user.id`, never a client-supplied `studentId`. Dashboard/Enrollllment-list/course-overview/derived weekly-schedule/resource-list-with-on-demand-signed-URLs/self-service profile read+narrow update. See ARCHITECTURE.md §25's neighboring §, DATABASE.md, SECURITY.md §3/§4/§6. |
